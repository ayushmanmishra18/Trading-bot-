...
async function main() {
  ...
  if (mongoose.connection.readyState !== 1) {
    console.log('No DB — seed skipped (in-memory mode needs no seed). Demo login: demo@tradepilot.app / demo1234');
    process.exit(0);
  }
  const User = require('./models/User');
  const Bot = require('./models/Bot');
  const email = 'demo@tradepilot.app';
  let u = await User.findOne({ email });
  if (!u) {
    u = await User.create({ name: 'Demo Trader', email, passwordHash: await bcrypt.hash('demo1234', 10), cash: 100000 });
